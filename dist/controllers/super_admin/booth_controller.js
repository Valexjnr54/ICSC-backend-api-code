...
exports.updateBooth = updateBooth;
exports.deleteBooth = deleteBooth;
exports.getBooths = getBooths;
exports.getBoothById = getBoothById;
const models_1 = require("../../models");
const express_validator_1 = require("express-validator");
const prisma = new models_1.PrismaClient();
async function createBooth(request, response) {
    ...
    try {
        ...
        const validationRules = [
            (0, express_validator_1.body)('booth_number').notEmpty().withMessage('Booth number is required'),
            (0, express_validator_1.body)('location').notEmpty().withMessage('Location is required'),
            (0, express_validator_1.body)('price').notEmpty().withMessage('Price is required').bail().isFloat().withMessage('Price must be a number'),
            (0, express_validator_1.body)('booth_size').notEmpty().withMessage('Booth size is required'),
            // Accept features as an array or a JSON-stringified array
            (0, express_validator_1.body)('features').custom((value) => {
...
async function updateBooth(request, response) {
    const admin_id = request.admin?.adminId;
    if (!admin_id)
        return response.status(403).json({ message: 'Unauthorized User' });
    const id = parseInt(request.query.booth_id, 10);
    if (!id)
        return response.status(403).json({ message: 'Booth ID required' });
    try {
        const check_admin = await prisma.admin.findUnique({ where: { id: admin_id } });
        if (check_admin?.role !== 'super_admin')
            return response.status(403).json({ message: 'Unauthorized User' });
        ...
        const validationRules = [
            (0, express_validator_1.body)('booth_number').optional().notEmpty().withMessage('Booth number is required'),
            (0, express_validator_1.body)('location').optional().notEmpty().withMessage('Location is required'),
            (0, express_validator_1.body)('price').optional().isFloat().withMessage('Price must be a number'),
            (0, express_validator_1.body)('booth_size').optional().notEmpty().withMessage('Booth size is required'),
            // Optional: if present must be array or JSON-stringified array
            (0, express_validator_1.body)('features').optional().custom((value) => {
                if (value === undefined)
                    return true;
                if (Array.isArray(value))
                    return true;
            ...
            (0, express_validator_1.body)('status').optional().isIn(['Available', 'SoldOut', 'Reserved', 'Paid']).withMessage('Invalid status'),
        ];
        await Promise.all(validationRules.map(rule => rule.run(request)));
        const errors = (0, express_validator_1.validationResult)(request);
        if (!errors.isEmpty())
...
async function deleteBooth(request, response) {
    const admin_id = request.admin?.adminId;
    if (!admin_id)
        return response.status(403).json({ message: 'Unauthorized User' });
    const id = parseInt(request.query.booth_id, 10);
...
async function getBooths(request, response) {
    const admin_id = request.admin?.adminId;
    if (!admin_id)
        return response.status(403).json({ message: 'Unauthorized User' });
    try {
...
async function getBoothById(request, response) {
    const admin_id = request.admin?.adminId;
    if (!admin_id)
        return response.status(403).json({ message: 'Unauthorized User' });
    const id = parseInt(request.query.booth_id, 10);
