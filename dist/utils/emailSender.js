...
function isTransientError(err) {
    ...
    if (transientMessages.some(t => msg.includes(t) || code === t))
        return true;
    return false;
}
function createTransporter() {
    ...
    if (!SMTP_HOST || !SMTP_PORT)
        throw new Error('SMTP configuration missing (SMTP_HOST/SMTP_PORT)');
    ...
    try {
        ...
        while (++attempt <= maxAttempts) {
            try {
                ...
                if (MAIL_DEBUG)
                    console.log(`SMTP send info (attempt ${attempt}):`, info);
                return true;
            }
            catch (err) {
                lastErr = err;
                ...
                if (isTransientError(err) && attempt < maxAttempts) {
                    ...
                    if (MAIL_DEBUG)
                        console.warn(`Transient SMTP error (attempt ${attempt}) - will retry after ${backoff}ms:`, err && err.message ? err.message : err);
                    await sleep(backoff);
